...
data class PolarisSpace(val id: String, val name: String, val state: String, val selected: Boolean)
data class PolarisSpaces(val enabled: Boolean, val available: Boolean, val canSwitch: Boolean,
    val selectedId: String, val spaces: List<PolarisSpace>) {
    ...
    companion object {
        ...
        fun parse(payload: String): PolarisSpaces? = runCatching {
            ...
            val spaces = (0 until array.length()).map { index ->
                ...
                require(idPattern.matches(id) && name.isNotBlank() && name.toByteArray(Charsets.UTF_8).size <= 128 &&
                    name.none { it.code < 32 || it.code == 127 } && state in states)
                PolarisSpace(id, name, state, entry.getBoolean("selected"))
            }
            require(spaces.map { it.id }.toSet().size == spaces.size)
            require(!available || enabled)
            require(!canSwitch || available)
            require(if (available) spaces.count { it.selected } == 1 && spaces.single { it.selected }.id == selectedId
                else selectedId.isEmpty() && spaces.none { it.selected })
            PolarisSpaces(enabled, available, canSwitch, selectedId, spaces)
        }.getOrNull()
    }
}
